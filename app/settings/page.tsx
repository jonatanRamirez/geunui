      ...
      <div className="space-y-4">
        ...
        <Section title={t(lang, "loyaltyPoints")}>
          <input
            ...
            onChange={(e) => setS({ ...s, loyaltyPoints: Number(e.target.value) })}
          />
        </Section>
        ...
        <Section title={t(lang, "widgetCount")}>
          <input
            ...
            className="w-full rounded-xl border px-3 py-2"
            value={s.widgetCount}
            onChange={(e) => setS({ ...s, widgetCount: Number(e.target.value) })}
          />
          <p className="mt-2 text-xs text-neutral-600">
            This controls how many widgets from the response are displayed.
          </p>
        </Section>
        ...
        <Section title="Context: geo ubicación">
          <div className="flex items-center justify-between gap-3">
            <label className="flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={s.geoEnabled}
                onChange={(e) => setS({ ...s, geoEnabled: e.target.checked })}
              />
              Enabled
            </label>
            <button
          ...
          {s.geoEnabled && s.geo && (
            <p className="mt-2 text-xs text-neutral-600">
              lat={s.geo.lat.toFixed(5)} lng={s.geo.lng.toFixed(5)} (±
              {Math.round(s.geo.accuracy ?? 0)}m)
            </p>
          )}
        </Section>
        ...
        <Section title={t(lang, "promptTemplate")}>
          <textarea
            ...
            onChange={(e) => setS({ ...s, promptTemplate: e.target.value })}
          />
          <div className="mt-2 flex flex-col gap-2 text-sm">
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={s.appendWidgetCount}
                onChange={(e) => setS({ ...s, appendWidgetCount: e.target.checked })}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={s.appendLoyaltyPoints}
                onChange={(e) => setS({ ...s, appendLoyaltyPoints: e.target.checked })}
              />
        ...
        <Section title="Shopping Muse">
          ...
          <textarea
            ...
            rows={6}
            value={s.selectorJson}
            onChange={(e) => setS({ ...s, selectorJson: e.target.value })}
          />
          <p className="mt-2 text-xs text-neutral-600">
            Provide the exact selector object expected by your account/campaign (JSON).
          </p>

          <label className="mt-3 block text-xs text-neutral-600">
            {t(lang, "apiKeyOverride")}
          </label>
          <input
            className="w-full rounded-xl border px-3 py-2"
            value={s.apiKeyOverride ?? ""}
            onChange={(e) => setS({ ...s, apiKeyOverride: e.target.value })}
            placeholder="Leave blank in production (use DY_API_KEY on Vercel)"
          />
          <p className="mt-2 text-xs text-amber-700">
            Avoid storing real API keys in the browser. Prefer server env vars.
          </p>
        </Section>
